...
class EnvCoordinator:
    """ Ensures that only one environment is used at a time. Lazily cleans up
        unused environments. """
    ...
    @inlineCallbacks
    def start_usage(self, env: Environment) -> Deferred:
    ...
    @inlineCallbacks
    def end_usage(self, env: Environment) -> Deferred:
        if self._active_env is not env:
            raise ValueError('end_usage called for wrong environment')
        yield self._lock.release()

    def auto_setup(self, env: Environment) -> Environment:
        coordinator = self
        ...
        class EnvSetupWrapper(Environment):
            ...
            def _start_usage(self) -> Deferred:
                ...
                try:
                    if self._num_users == 0:
                        yield coordinator.start_usage(env)
                    self._num_users += 1
                finally:
                    self._lock.release()
            ...
            def _end_usage(self) -> Deferred:
                ...
                try:
                    self._num_users -= 1
                    if self._num_users == 0:
                        yield coordinator.end_usage(env)
                finally:
                    self._lock.release()
